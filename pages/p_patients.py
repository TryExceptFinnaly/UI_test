...
class PatientsPage(MainContentPage):
    ...
    def check_found_patient(self):
        ...
        return True if self.element_is_visible(
            PatientSearchLocators.get_visit_locator(name=full_name, birthdate=birthdate, sex=sex,
    ...
    def check_found_patient_in_edit_tab(self):
        ...
        sex = SystemDirectory.sex[self.patient_info.sex][0]
        assert self.element_is_visible(PatientLocators.EditTab.INPUT_FIRST_NAME).get_attribute(
            'value') == self.patient_info.first_name
        assert self.element_is_visible(PatientLocators.EditTab.INPUT_LAST_NAME).get_attribute(
            'value') == self.patient_info.last_name
        assert self.element_is_visible(PatientLocators.EditTab.INPUT_MIDDLE_NAME).get_attribute(
            'value') == self.patient_info.middle_name
